...
function tranform (table, aId, name , tableAll) {
        ...
        while (i < len) {
            if (i === -1) {
                Array.from(tableHead.rows[0].children).forEach((td) => {
                    if (td.children[0].children[0].innerHTML !== '#'){
...
let tableToExcel = (function () {
    ...
    let template = '<html><head><meta charset="UTF-8"></head><body><table border="1">{table}</table></body></html>';
