...
export default {
    ...
    methods: {
            ...
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    var param = {
                        ...
                        oldPwd: _this.editPasswordForm.oldPass,
                        newPwd: _this.editPasswordForm.newPass
                    }
                    Util.postApiData(param , '/BackStage/Administration/UpdatePwd').then((res) => {
                        if(res.state == 1) {
                            ...
                            _this.editPasswordForm.oldPass = '';
                            _this.editPasswordForm.newPass = '';
                            _this.editPasswordForm.rePass = '';
        ...
        init () {
            ...
            Util.getAjaxData(param , 'QueryAdminList' , undefined ).then((res)=> {
                var user = res[0]
                _this.userForm.name = user.Account;
                _this.userForm.cellphone = user.Mobile;
                _this.initPhone = user.Mobile;
                _this.userForm.company = '印生活网络科技有限公司';
                _this.userForm.department = user.RoleName;
            })
        },
        cancelInputCodeBox () {
        ...
        saveInfoAjax () {
            ...
            var param = {
                Id: _this.userInfo.userId,
                Account: _this.userForm.name,
                Mobile: _this.userForm.cellphone
            }

            Util.postApiData(param , '/BackStage/Administration/UpdateUser').then((res) => {
                if(res.state == 1){
                    _this.$Notice.success({
                        title: '提示',
                        desc:  res.msg
