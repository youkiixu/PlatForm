    ...
    <div @keyup.enter="getData()">
        ...
        <Card>
            ...
            <Row class="margin-top-10">
                <Col span="4">
                起点:
                <Input v-model="form.$StartPoint" placeholder="请输入关键字..." style="width: 150px" />
                </Col>
                <Col span="4">
                终点:
                <Input v-model="form.$EndPoint" placeholder="请输入关键字..." style="width: 150px" />
                </Col>
                <Col span="16">
                时间日期:
                <datePickerCompoent
        ...
        <addDistributionSetManagenetComponent ref="showEdit" :param="selectRowData" @onSuccess="getData()"></addDistributionSetManagenetComponent>
...
<script>
    ...
    // 添加单品重量，修改单品重量
    import addDistributionSetManagenetComponent from './components/addDistributionSetManagenet-component'


    export default {
        name: 'DistributionSetManagenetView',
        components: {
            datePickerCompoent,
            addDistributionSetManagenetComponent,
        ...
        data () {
            return {
                form: {
                    ...
                    $pageRecord: 15,
                    $rowIndex : 1,
                    strMethod: 'QueryDistributionSetList'
        ...
        methods: {
            ...
            delRow() {
                let _this = this;
                let obj = this.selectRowData
                if(obj == undefined) {
                    _this.$Message.warning('请选择一行内容!');
                }
                this.$Modal.confirm({
                    title: '确定删除？',
                    content: '是否确认删除 编号为：【' + obj.Id + '】的分配' + obj.StartPoint + ' - ' + obj.EndPoint + '地址信息？',
                    onOk: function() {
                        var par = {
                            'Id': obj.Id,
                            'rowState' : 'D'
