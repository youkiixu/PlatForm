...
<template> 
	<div>
		省:
		<Select  style="width:100px" v-model="city1" @on-change="city1Change">
			<Option v-for="item in cityList1" :value="item.value" :key="item.value">{{ item.label }}</Option>
		</Select>
		市:
		<Select  style="width:100px" v-model="city2" @on-change="city2Change" clearable>
			<Option v-for="item in cityList2" :value="item.value" :key="item.value">{{ item.label }}</Option>
		</Select>
		县:
		<Select  style="width:150px" v-model="city3" @on-change="city3Change" clearable>
...
<script>
	...
	export default {
		...
		methods: {
			...
			city3Change(data) {
				this.$emit('valueChange' , [this.city1, this.city2 , this.city3])
			},
			getData() {
                // console.log(data)
                var cityName = ''
