<template>
  ...
  <script>
    ...
    export default {
      name: 'areaDistributeComponent',
      components: {
        areaIndex
      },
      data: function () {
        return {
        chinaArea: [], // 这是地区联动的JSON
        notLimitButton: {
    ...
    props: {
      ...
      queryRoadMethod: {
        type: String,
        default: 'QueryDeliveryAddressMap'
      },
      transfer: {
        type: Boolean,
        default: false //父组件调用transfer，设成true,不在执行本组件的transfer
      },
      level: {
        type: Number,
        default: 5
    ...
    methods: {
      getSelectList () {
        var _this = this;
        var paramString = this.selectedParam
        var par = {}
        par[paramString] = this.value
        ...
        Util.getAjaxData(par, _this.selectedListMethod).then((res) => {
          ...
          res.map((item)=>{
            item.selected = false
            item.strText = (item.strCity ?item.strCity : '')  + (item.strArea ? item.strArea : '') + (item.strStreet ?item.strStreet : '') + (item.strRoad ?item.strRoad : '' )
      ...
      getRoadList(id) {
        ...
        this.resetList()
        this.$Loading.start();
        Util.getAjaxData(par , _this.queryRoadMethod).then((res) => {
      ...
      transferToRight() {
        ...
        this.$emit('on-transfer-right' , _this.roadList)
      },
      transferToLeft() {
        // 删除操作
        var _this = this;
        ...
        _this.selectedList.map((item) => {
          // status=1是已经有配送员的区域 selected是已经选中
          if(item.selected){
            arr.push({rowState: "D" , wayId: item.strCode , id: item.id})
        ...
        if(arr.length == 0) return;
        this.$emit('on-transfer-left' , _this.selectedList)
