<template>
	...
	@focus="startChoice" @keypress="startChoice"  ref="input" > -->
		<Input v-model="showName" @on-focus="startChoice" @on-keypress="startChoice" @on-blur="startChoice" :readonly="readonly" ref="input" placeholder="请选择地址" style="width: 410px;margin: 0 10px;cursor: pointer"></Input>
		<!--选择面板-->
		<!-- v-show="distPanelIsShow" -->
		<div class="vdp-panel" v-if="showSelect">
			<h5>选择省市区</h5>
			<ul class="vdp-tags">
...
export default {
  ...
  methods: {
    ...
    getAreaData(code) {
      ...
      return Util.getAjaxData(par, "QueryDeliveryAddressMap", undefined).then(
        res => {
    ...
    setShowName() {
      let { currProv, currCity, currDist, currStreet } = this;
      this.showName =
        currProv + " " + currCity + " " + currDist + " " + currStreet;
    },
    // 立即关闭省市区面板
    immEndChoice() {
